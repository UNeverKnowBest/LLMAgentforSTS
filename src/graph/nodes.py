    ...
    try:
        ...
        if command:
            ...
            try:
                ...
                final_command = (
                    response.command.strip() if response.command else "state"
                )
    ...
    if available_commands:
        ...
        if any(cmd.lower() == command_base for cmd in available_commands):
            validation_result["warnings"].append(
                f"Command base '{command_base}' found in available commands"
            )
        else:
            validation_result["warnings"].append(
                f"Command base '{command_base}' not explicitly in available commands: {available_commands}"
            )
    ...
    if command_word == "play":
        if len(tokens) < 2:
            ...
            validation_result["errors"].append(
                "play command card index must be a number"
            )
        else:
            card_index = int(tokens[1])
            if card_index == 0:
                validation_result["warnings"].append(
                    "Card index 0 will be treated as 10"
                )
            elif card_index < 0:
                validation_result["errors"].append(
                    "play command card index must be non-negative"
                )

            if len(tokens) == 3:
                if not tokens[2].isdigit():
                    validation_result["errors"].append(
                        "play command target index must be a number"
                    )
                elif int(tokens[2]) < 0:
                    validation_result["errors"].append(
                        "play command target index must be non-negative"
                    )
            elif len(tokens) > 3:
                validation_result["errors"].append(
                    "play command has too many arguments"
                )
        ...
        if len(tokens) < 2:
            validation_result["errors"].append(
                "choose command missing choice parameter"
            )
        else:
            validation_result["is_valid"] = True
        ...
        if len(tokens) < 3:
            validation_result["errors"].append(
                "potion command format: potion use/discard [index] [target]"
            )
        elif tokens[1].lower() not in ["use", "discard"]:
            validation_result["errors"].append(
                "potion command second parameter must be 'use' or 'discard'"
            )
        elif not tokens[2].isdigit():
            validation_result["errors"].append(
                "potion command potion index must be a number"
            )
        elif int(tokens[2]) < 0:
            validation_result["errors"].append(
                "potion command potion index must be non-negative"
            )
        elif len(tokens) == 4 and not tokens[3].isdigit():
            validation_result["errors"].append(
                "potion command target index must be a number"
            )
        elif len(tokens) == 4 and int(tokens[3]) < 0:
            validation_result["errors"].append(
                "potion command target index must be non-negative"
            )
        elif len(tokens) > 4:
        ...
        if len(tokens) > 1:
            validation_result["errors"].append(
                f"{command_word} command does not need arguments"
            )
        else:
            validation_result["is_valid"] = True
        ...
        if len(tokens) < 2:
            validation_result["errors"].append(
                "start command missing character parameter"
            )
        else:
            ...
            if character not in valid_characters:
                validation_result["warnings"].append(
                    f"Character '{tokens[1]}' may not be recognized by CommunicationMod"
                )
            ...
            if len(tokens) >= 3:
                try:
                    ...
                    if ascension_level < 0 or ascension_level > 20:
                        validation_result["errors"].append(
                            "start command ascension level must be between 0 and 20"
                        )
                except ValueError:
                    validation_result["errors"].append(
                        "start command ascension level must be a number"
                    )
            ...
            if len(tokens) >= 4:
                ...
                if not re.match(r"^[A-Z0-9]+$", seed):
                    validation_result["errors"].append(
                        "start command seed must be alphanumeric"
                    )

            if len(tokens) > 4:
                validation_result["errors"].append(
                    "start command has too many arguments"
                )
        ...
        if len(tokens) < 2:
            ...
            if key_name not in valid_keys:
                validation_result["errors"].append(
                    f"key command key invalid: {tokens[1]}"
                )
            ...
            if len(tokens) >= 3:
                try:
                    ...
                    if timeout < 0:
                        validation_result["errors"].append(
                            "key command timeout must be non-negative"
                        )
                except ValueError:
                    validation_result["errors"].append(
                        "key command timeout must be a number"
                    )
        ...
        if len(tokens) < 4:
            validation_result["errors"].append(
                "click command format: click left/right [x] [y] [timeout]"
            )
        elif tokens[1].upper() not in ["LEFT", "RIGHT"]:
            validation_result["errors"].append(
                "click command button parameter must be 'left' or 'right'"
            )
        else:
            try:
                x = float(tokens[2])
                ...
                if x < 0 or x > 1920:
                    validation_result["warnings"].append(
                        "X coordinate outside standard range (0-1920)"
                    )
                if y < 0 or y > 1080:
                    validation_result["warnings"].append(
                        "Y coordinate outside standard range (0-1080)"
                    )
                ...
                if len(tokens) >= 5:
                    try:
                        ...
                        if timeout < 0:
                            validation_result["errors"].append(
                                "click command timeout must be non-negative"
                            )
                    except ValueError:
                        validation_result["errors"].append(
                            "click command timeout must be a number"
                        )

                if len(tokens) > 5:
                    validation_result["errors"].append(
                        "click command has too many arguments"
                    )

            except ValueError:
                validation_result["errors"].append(
                    "click command coordinates must be numbers"
                )
        ...
        if len(tokens) < 2:
            ...
            try:
                ...
                if timeout < 0:
                    validation_result["errors"].append(
                        "wait command time must be non-negative"
                    )
            except ValueError:
            ...
            if len(tokens) > 2:
                validation_result["errors"].append(
                    "wait command has too many arguments"
                )

        if not validation_result["errors"]:
            validation_result["is_valid"] = True

    validation_result["screen_type"] = (
        current_screen_type.value if current_screen_type else "UNKNOWN"
    )
...
def handle_validation_failure(state: GameState):
    ...
    if not validation_result:
        print(
            f"Error: No validation result found for command: {final_command}",
            file=sys.stderr,
        )
...
def generate_fallback_command(
    screen_type: str, original_command: str, errors: list
) -> str:
    safe_commands = {
        "MAP": "state",
        "CARD_REWARD": "skip",
